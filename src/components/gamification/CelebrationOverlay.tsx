'use client';
/**
 * CelebrationOverlay.tsx — Confetti + Star burst animation
 * Ported from LinguaKids — CSS-only, no external deps
 * Types: correct | levelup | streak | perfect | badge
 */
...
type CelebType = 'correct' | 'levelup' | 'streak' | 'perfect' | 'badge';
...
export default function CelebrationOverlay({ type = 'correct', message = '', messageVi = '', onDone, xpGained = 0 }: Props) {
  ...
  useEffect(() => {
    const t = setTimeout(() => { setVisible(false); onDone?.(); }, type === 'levelup' ? 3000 : 2000);
    return () => clearTimeout(t);
  }, [type, onDone]);

  if (!visible) return null;

  const configs: Record<CelebType, { emoji: string; bg: string; label: string; labelVi: string }> = {
  ...
  return (
    <div style={{
      ...
      <div style={{ position: 'fixed', inset: 0, overflow: 'hidden', pointerEvents: 'none' }}>
        {Array.from({ length: type === 'levelup' ? 40 : 20 }).map((_, i) => (
          <ConfettiPiece key={i} delay={Math.random() * 0.5}
            color={CONFETTI_COLORS[i % CONFETTI_COLORS.length]} left={Math.random() * 100} />
        ))}
      </div>
      ...
      <div style={{
        ...
        <div style={{ fontSize: '1.4rem', fontWeight: 800, marginBottom: '4px' }}>{cfg.label}</div>
        <div style={{ fontSize: '0.85rem', opacity: 0.9 }}>{cfg.labelVi}</div>
