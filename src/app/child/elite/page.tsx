...
export default function EliteDashboardPage() {
    ...
    return (
        <div className="page-container" style={{ paddingTop: '1rem', background: 'var(--color-bg-child)', minHeight: '100vh' }}>
            {/* Header */}
            <div className="page-header" style={{ marginBottom: '1rem' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    <div>
                        <Link href="/child" style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', color: 'var(--color-text-secondary)', textDecoration: 'none', marginBottom: '0.5rem' }}>
                            <ArrowLeft size={16} /> {t('back')}
                        </Link>
                        <h1 className="page-title" style={{ fontSize: '1.5rem' }}>{t('elite_page_title')}</h1>
                        <p className="page-subtitle">{t('elite_subtitle')}</p>
                    </div>
                    <LangToggle />
                </div>
            </div>
            ...
            {activeTab === 'probability' && probIndex < probScenarios.length && (
                <div className="card animate-fade-in">
                    <h2 style={{ fontWeight: 700, marginBottom: '0.5rem' }}>{t('prob_title')}</h2>
                    <p style={{ color: 'var(--color-text-secondary)', fontSize: '0.85rem', marginBottom: '1rem' }}>
                        {t('q_label')} {probIndex + 1} / {probScenarios.length} — {t('score_label')}: {probScore}
                    </p>
                        ...
                        {probScenarios[probIndex].options.map((opt) => {
                            ...
                            return (
                                <button key={opt} onClick={() => { if (probAnswer) return; setProbAnswer(opt); if (opt === probScenarios[probIndex].correctAnswer) setProbScore((s) => s + 20); }}
                                    className="btn" style={{ textAlign: 'left', padding: '0.75rem 1rem', background: isCorrect ? '#d1fae5' : isWrong ? '#fee2e2' : 'white', border: isCorrect ? '2px solid #10b981' : isWrong ? '2px solid #ef4444' : '2px solid var(--color-border)', fontWeight: 600 }}>
                                    {likelihoodLabel[opt]}
                                </button>
                            );
                        })}
                    ...
                    {probAnswer && (
                        <div style={{ marginTop: '1rem', padding: '1rem', background: '#f0fdf4', borderRadius: '12px' }}>
                            <p style={{ fontWeight: 600, marginBottom: '0.5rem' }}>{probAnswer === probScenarios[probIndex].correctAnswer ? t('correct') : t('wrong')}</p>
                            <p style={{ fontSize: '0.9rem', color: 'var(--color-text-secondary)' }}>{probScenarios[probIndex].explanation}</p>
                            <button className="btn btn-primary" style={{ marginTop: '0.75rem' }} onClick={() => { setProbAnswer(null); setProbIndex((i) => i + 1); }}>{t('next')}</button>
                        </div>
                    )}
                </div>
            )}
            {activeTab === 'probability' && probIndex >= probScenarios.length && (
                <div className="card animate-fade-in" style={{ textAlign: 'center' }}>
                    ...
                    <h2 style={{ fontWeight: 700 }}>{t('done')} {t('score_label')}: {probScore}/100</h2>
                    <button className="btn btn-primary" style={{ marginTop: '1rem' }} onClick={() => { setProbIndex(0); setProbScore(0); setProbAnswer(null); }}>{t('replay')}</button>
                </div>
            )}
            ...
            {activeTab === 'finance' && wvnIndex < WANTS_VS_NEEDS.length && (
                <div className="card animate-fade-in">
                    <h2 style={{ fontWeight: 700, marginBottom: '0.5rem' }}>{t('fin_title')}</h2>
                    <p style={{ color: 'var(--color-text-secondary)', fontSize: '0.85rem', marginBottom: '1rem' }}>
                        {t('q_label')} {wvnIndex + 1} / {WANTS_VS_NEEDS.length} — {t('score_label')}: {wvnScore}
                    </p>
                    <div style={{ textAlign: 'center', fontSize: '4rem', marginBottom: '0.5rem' }}>{WANTS_VS_NEEDS[wvnIndex].visual}</div>
                    <p style={{ fontWeight: 600, textAlign: 'center', marginBottom: '1rem' }}>{WANTS_VS_NEEDS[wvnIndex].name}</p>
                        ...
                        {(['want', 'need'] as const).map((opt) => {
                            ...
                            return (
                                <button key={opt} onClick={() => { if (wvnAnswer) return; setWvnAnswer(opt); if (opt === WANTS_VS_NEEDS[wvnIndex].correctCategory) setWvnScore((s) => s + 12); }}
                                    className="btn" style={{ flex: 1, padding: '1rem', fontSize: '1.1rem', fontWeight: 700, background: isCorrect ? '#d1fae5' : isWrong ? '#fee2e2' : opt === 'want' ? '#fef3c7' : '#dbeafe', border: isCorrect ? '2px solid #10b981' : isWrong ? '2px solid #ef4444' : '2px solid transparent' }}>
                                    {opt === 'want' ? t('fin_want') : t('fin_need')}
                                </button>
                            );
                        })}
                    </div>
