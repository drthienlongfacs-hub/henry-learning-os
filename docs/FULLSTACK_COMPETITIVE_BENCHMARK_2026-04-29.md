...
Henry Learning OS đã vượt mức "display demo" ở các phần: AI gia sư, mastery/adaptive, dashboard bằng chứng, safety settings, static build và GitHub Pages. Điểm cạnh tranh nội bộ hiện là **65/100** theo scorecard có trọng số sau khi có SOT Control Center, AI tutor rubric 50 scenario, RCA/PDCA tuần, privacy evidence panel và human review queue.
...
Nâng cấp mới đã hoàn tất ở mức item review workflow: **47/47 topic lớp 1-5** trong app đã có curriculum map, generated item có `curriculumMapId`, và toàn bộ được đưa vào human review queue với RCA/PDCA, reviewer metadata schema và release gate. Chưa được claim item bank phủ chuẩn cho đến khi có người duyệt thật và calibration bằng dữ liệu làm bài thật.

UI/UX live đã được chỉnh để khác biệt thấy ngay: benchmark hiện điểm 65/100, trang `/parent/sot` hiển thị SOT Control Center, `/parent/review-queue` hiển thị hàng đợi duyệt item lớp 1-5, dashboard phụ huynh có RCA/PDCA tuần, settings có privacy evidence panel, nhưng vẫn cảnh báo chưa claim hiệu quả học tập.
...
| Hạ tầng dữ liệu | 9% | 6.4/10 | Local-only, chưa có reviewer decisions thật/cohort analytics | Analytics export schema và reviewer decision persistence |
| Build/test/deploy | 6% | 7.0/10 | Chưa có E2E CI | Playwright smoke trong CI |

## Roadmap đúng trọng tâm

1. **Diagnostic warm-start lớp 1**: xác định level ban đầu và kế hoạch 7 ngày bằng dữ liệu bài làm.
2. **Playwright/WCAG smoke CI**: kiểm route live, mobile, focus và no blank page.
3. **Pilot evidence pack 4 tuần**: pre/post, retention sau 7 ngày, time-on-task, lỗi tái phát, cohort dashboard.
4. **Reviewer decision persistence và calibration**: lưu reviewerId/approvedAt/blockReason thật và đưa item approved vào calibration.
