...
Henry Learning OS đã vượt mức "display demo" ở các phần: AI gia sư, mastery/adaptive, dashboard bằng chứng, safety settings, static build và GitHub Pages. Điểm cạnh tranh nội bộ hiện là **64/100** theo scorecard có trọng số sau khi có SOT Control Center, AI tutor rubric 50 scenario, RCA/PDCA tuần và privacy evidence panel.
...
Nâng cấp tiếp theo đã hoàn tất ở mức item traceability: **47/47 topic lớp 1-5** trong app đã có curriculum map, và generated item trong luồng học được gắn `curriculumMapId`, source version, official strand, review status và calibration status. Chưa được claim item bank phủ chuẩn cho đến khi có người duyệt và calibration bằng dữ liệu làm bài thật.

UI/UX live đã được chỉnh để khác biệt thấy ngay: benchmark hiện điểm 64/100, trang `/parent/sot` hiển thị SOT Control Center, dashboard phụ huynh có RCA/PDCA tuần, settings có privacy evidence panel, nhưng vẫn cảnh báo chưa claim hiệu quả học tập.
...
| Người duyệt nội dung | Đang làm | Item đã có `reviewStatus` | Review queue, `reviewerId`, `approvedAt`, lý do block |
...
| Vận hành production | Đang làm | Có Pages deploy và unit/build gate | Playwright smoke CI, monitoring live, export cohort |
...
| 3 | 8 | Toán nhân/chia, chu vi, quy luật; Tiếng Việt ngữ pháp; Ngoại ngữ 1 từ vựng; Tin học phần cứng | Review queue cho người duyệt item |
...
| Độ sâu chương trình | 11% | 8.0/10 | Đã có `curriculumMapId` cho generated item, nhưng chưa có người duyệt/calibration | Review queue và khóa release item chưa duyệt |
...
| Hạ tầng dữ liệu | 9% | 6.1/10 | Local-only, chưa có reviewer queue/cohort analytics | Analytics export schema và reviewer queue |
| Build/test/deploy | 6% | 7.0/10 | Chưa có E2E CI | Playwright smoke trong CI |

## Roadmap đúng trọng tâm

1. **Human review queue**: reviewerId, approvedAt, block reason và gate không claim item bank approved khi chưa duyệt.
2. **Diagnostic warm-start theo môn**: xác định level ban đầu và kế hoạch 7 ngày bằng dữ liệu bài làm.
3. **Playwright/WCAG smoke CI**: kiểm route live, mobile, focus và no blank page.
4. **Pilot evidence pack 4 tuần**: pre/post, retention sau 7 ngày, time-on-task, lỗi tái phát, cohort dashboard.
